...
(function () {
  ...
  route('/lesson/:id', async (id, params, token) => {
    ...
    const el = h(`<div class="lesson track-${m.track}" id="lesson">${backLink('#/module/' + m.id, m.title)}<div class="ink-bar" id="ink-bar" hidden></div>
      ...
      ${(l.videos || []).length ? `<section class="watch"><div class="eyebrow accent">Watch</div><div class="videos">${l.videos.map((v) => `<figure class="video"><div class="frame"><iframe src="https://www.youtube-nocookie.com/embed/${esc(v.id)}?rel=0&modestbranding=1${v.start ? '&start=' + v.start : ''}" title="${esc(v.title)}" loading="lazy" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><figcaption class="small"><b>${esc(v.title)}</b> <span class="faint">· ${esc(v.channel)}</span></figcaption></figure>`).join('')}</div><p class="faint tiny" style="margin-top:6px">Outside videos, chosen to match this lesson. Where a video and your textbook disagree, the textbook wins.</p></section>` : ''}
  ...
  route('/settings', (params, token) => {
    ...
    const el = h(`<div class="settings"><div class="page-head"><div><div class="eyebrow">Rounds</div><h1>Settings</h1></div></div>
      ...
      <div class="card"><div class="eyebrow">Sync</div><p class="small muted" style="margin-top:8px">Progress syncs between your devices through the homelab whenever the app is open on Tailscale. Lessons, cards, quizzes, hours, mistakes, courses, and the degree plan all merge; the more advanced record wins. Pencil notes and station videos stay on the device they were made on.</p><div class="row" style="margin-top:10px"><span class="chip" id="sync-status">…</span><span class="faint small">this device: ${esc(window.Sync ? Sync.device : '')}</span></div><div class="btn-row" style="margin-top:10px"><button class="btn sm" id="sync-now">Sync now</button></div></div>
    ...
    const syncChip = () => { const c = $('#sync-status', el); if (!c || !window.Sync) return; const st = Sync.status(); c.textContent = st === 'ok' ? 'Synced' : st === 'offline' ? 'Not reachable (needs Tailscale)' : 'Checking…'; c.className = 'chip ' + (st === 'ok' ? 'good' : st === 'offline' ? 'warn' : ''); };
    syncChip(); if (window.Sync) Sync.onStatus(syncChip);
    $('#sync-now', el).onclick = async () => { if (!window.Sync) return; const changed = await Sync.pull(); await Sync.push(); syncChip(); toast(changed ? 'Merged progress from your other device.' : (Sync.status() === 'ok' ? 'Up to date.' : 'Could not reach the homelab.')); if (changed) navigate(); };
  ...
  // ---------- Boot ----------
  applyTheme();
  window.Rounds = { rerender: () => navigate() };
  if (!IDX.modules.length) view.innerHTML = '<div class="empty">No content loaded. Run the build.</div>';
  // Pull progress from the homelab first (fast on the tailnet; gives up quickly when offline), then render.
  (window.Sync ? Promise.race([Sync.pull(), new Promise((r) => setTimeout(r, 2500))]) : Promise.resolve()).then(() => navigate(), () => navigate());
